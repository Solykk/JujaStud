 ...
 * где каждый элемент C[i][j] высчитывается по формуле: C[i][j] = A[i][0]*B[0][j] + A[i][1]*B[1][j] + ...
 * + A[i][N-1]*B[N-1][j]
 * Пример:
 * |1 2|   |3 4 5 6|   |21 4  7  10|
 * |7 8| * |9 0 1 2| = |93 28 43 58|
 * |3 4|               |45 12 19 26|
 * так как 1*3 + 2*9 = 21, 1*4 + 2*0 = 4, 1*5 + 2*1 = 7, 1*6 + 2*2 = 10 7*3 + 8*9 = 93,
 * 7*4 + 8*0 = 28, 7*5 + 8*1 = 43, 7*6 + 8*2 = 58 3*3 + 4*9 = 45, 3*4 + 4*0 = 12, 3*5 + 4*1 = 19,
 * 3*6 + 4*2 = 26
...
public class Lab21 {
    public static int[][] mul(int[][] fst, int[][] snd) {
        int[][] res = new int [fst.length][snd[0].length];
