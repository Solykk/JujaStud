 ...
 * Рекурсивное решение
 *
 * Пронумеруем пирамидки - A, где изначально все диски, B, C. Нам надо перенести все диски с A -> C, B - промежуточная
 * пирамида. Также обозначим диски в соответствии с радиусом - 3 2 1. 1 - наименьший. Изначальная позиция A: 321, B, C.
 * Рекурсия заключается в следующем - переносим все диски кроме последнего (в более общем случае n-1 дисков)
 * на промежуточный B, переносим последний диск на C, переносим все диски кроме последнего на C.
 * Теперь вместо переноса 3х дисков и 2х свободных пирамидок, нам нужно решить задачу переноса 2х дисков и
 * 2х свободных ячеек (мы можем класть диски меньшего радиуса на диск большего).
 * exchange(21, A, B)
 * exchange(3, A, C)
 * exchange(21, B, C)
 * Функция exchange переносит диск, если он один или запускает рекурсивную процедуру, если дисков больше одного.
 * Напомню, что атомарным действием есть перенос 1 диска. Поэтому для переноса дисков 21 запускаем аналогичный алгоритм.
 * exchange(1, A, C)
 * exchange(2, A, B)
 * exchange(1, C, B)
 * Посмотрим шаги:
 ...
 *      }
 * }
 * Про Stack мы детальнее поговорим в следующих уроках. Пока что это структура данных, которая хранит множество
 * элементов и реализует принцип LIFO (last in first out) - последний пришел, первый ушел. Тоесть,
 * когда мы просим стэк отдать элемент методом pop(), он возвращает последний добавленный элемент.
